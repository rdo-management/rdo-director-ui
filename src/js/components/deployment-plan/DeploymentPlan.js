...
export default class DeploymentPlan extends React.Component {
  ...
  render() {
    let deploymentConfigLinks = [
      <Link className="btn btn-link" to={'/' + this.props.route.path + '/environment'}>Edit Configuration</Link>
    ];

    let roleConfigLinks = [
      <Link className="btn btn-link" to={'/' + this.props.route.path + '/parameters'}>Edit Parameters</Link>
    ];
          ...
          {this.state.currentPlanName || true? (
            <div className="col-sm-12">
              ...
              <ol className="deployment-step-list">
                <DeploymentStep stepNumber={1}
                                title="Specify Deployment Configuration"
                                subTitle={deploymentConfigDescription}
                                links={deploymentConfigLinks}/>
                <DeploymentStep stepNumber={2}
                                title="Create Flavors and Register Nodes"
                                subTitle={this.state.flavors.length > 0 ?
                                 '' : 'There are no flavors or nodes currently.'} />
                <DeploymentStep stepNumber={3} title="Configure and Assign Roles"
                                subTitle="Parameters for all roles can be configured."
                                links={roleConfigLinks}/>
                <DeploymentStep stepNumber={4} title="Deploy">
